...
class CommicSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if response.status == 200 and '检测到有异常' not in response.text:
            with open(str(response.url).replace("https://book.douban.com/subject/", "").strip('/') + ".html",
                      'w+') as fp:
                fp.write(response.body.decode("utf8"))
                fp.close()
